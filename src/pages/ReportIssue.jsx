...
import { fallbackCategorize } from '../services/ai';
import { azureCategorize, hasAzureOpenAIConfig } from '../services/ai/azure';
...
export default function ReportIssue() {
  ...
  const handlePhoto = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      // Pass the actual image data URL so Azure OpenAI can inspect the image
      runAI(file.name, dataUrl);
    };
    reader.readAsDataURL(file);
  };

  const runAI = async (filename, imageDataUrl = null) => {
    setAiAnalyzing(true);
    setAiError('');
    try {
      if (hasAzureOpenAIConfig()) {
        // Azure OpenAI Vision — uses the uploaded image directly
        const result = await azureCategorize(description || 'civic issue', imageDataUrl);
        setAiResult({ ...result, confidence: 0.9 });
        ...
        // Keyword fallback — may not work on mobile with generic filenames
      ...
      // Show user-friendly error and fall back to keyword detection
      if (err.message.includes('429') || err.message.includes('quota')) {
        setAiError('⚠️ Azure OpenAI quota exceeded. Using keyword detection instead. Try again in a few minutes.');
      } else {
        setAiError(`⚠️ AI detection failed: ${err.message}. Using keyword detection instead.`);
      }
  ...
  const handleAIRewrite = async () => {
    // AI Rewrite is currently disabled (requires API key)
    return;
  };
        ...
        <div className="report-step-content">
          ...
          {aiAnalyzing && (
            <div className="ai-analyzing glass-card">
              ...
              <span className="ai-powered-badge">{hasAzureOpenAIConfig() ? 'Powered by Azure OpenAI' : 'Analyzing...'}</span>
            </div>
          )}

          {aiError && !aiAnalyzing && (
            <div className="ai-error glass-card" style={{borderLeft:'3px solid var(--color-warning)', background:'var(--color-warning-bg, #fff3cd)', padding:'10px 14px', marginBottom:8, borderRadius:10}}>
              <p style={{margin:0, fontSize:13, color:'var(--text-primary)'}}>{aiError}</p>
            </div>
          )}
          ...
          {aiResult && !aiAnalyzing && (
            <div className="ai-result glass-card">
              <span className="ai-powered-badge">
                {hasAzureOpenAIConfig() && !aiError ? '✨ Azure OpenAI' : '✨ AUTO-DETECT'}
              </span>
              {aiResult.confidence > 0.35 ? (
                <p className="ai-detected">Detected: <strong>{categories.find(c => c.id === aiResult.category)?.icon} {aiResult.category}</strong></p>
              ) : (
                <p className="ai-detected">📷 Photo received — please <strong>select a category</strong> in the next step</p>
              )}
