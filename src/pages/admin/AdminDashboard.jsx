                  ...
                  <span className="ad-col-status">
                    <div style={{display:'flex', gap: 6, flexWrap: 'wrap'}}>
                      {issue.status === 'reported' && (
                        <>
                          <button className="btn btn-sm btn-primary" onClick={() => handleStatusChange(issue.id, 'acknowledged')} style={{fontSize: 11}}>✓ Ack</button>
                          <button className="btn btn-sm btn-outline" onClick={() => handleStatusChange(issue.id, 'in-progress')} style={{fontSize: 11}}>🔄 Start</button>
                        </>
                      )}
                      {issue.status === 'acknowledged' && (
                        <>
                          <button className="btn btn-sm btn-primary" onClick={() => handleStatusChange(issue.id, 'in-progress')} style={{fontSize: 11}}>🔄 Progress</button>
                          <button className="btn btn-sm btn-outline" onClick={() => handleStatusChange(issue.id, 'escalated')} style={{fontSize: 11}}>⬆ Escalate</button>
                        </>
                      )}
                      {issue.status === 'in-progress' && (
                        <>
                          <button className="btn btn-sm btn-success" onClick={() => handleStatusChange(issue.id, 'resolved')} style={{fontSize: 11}}>✅ Resolve</button>
                          <button className="btn btn-sm btn-outline" onClick={() => handleStatusChange(issue.id, 'escalated')} style={{fontSize: 11}}>⬆ Escalate</button>
                        </>
                      )}
                      {issue.status === 'resolved' && (
                        <span className="tag tag-success" style={{fontSize: 11}}>✅ Resolved</span>
                      )}
                      {issue.status === 'escalated' && (
                        <>
                          <span className="tag tag-warning" style={{fontSize: 11}}>⬆ Escalated</span>
                          <button className="btn btn-sm btn-outline" onClick={() => handleStatusChange(issue.id, 'in-progress')} style={{fontSize: 11}}>🔄 Back</button>
                        </>
                      )}
                    </div>
                  </span>
